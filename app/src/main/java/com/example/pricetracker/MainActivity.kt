...
class MainActivity : AppCompatActivity() {
    ...
    // Кэш хранит не только точки, но и массив дат для оси X
    ...
    private fun setupChart() {
        ...
        // Настройка оси Y (без лишних нулей)
    ...
    private fun fullSync() {
        tvSt?.text = "Загрузка: XAU, XAG, BTC..."
        listOf("XAUEUR=X", "XAGEUR=X", "BTC-EUR").forEach { fetchYahoo(it) }
    }

    private fun fetchYahoo(symbol: String) {
        // Умные интервалы: 1D = 5 минут, 1W = 15 минут. Иначе будет прямая линия!
        val range = when(days) { 1->"1d"; 7->"5d"; 30->"1mo"; 365->"1y"; else->"5y" }
        val interval = when(days) { 1->"5m"; 7->"15m"; 30->"1d"; 365->"1d"; else->"1wk" }
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    runOnUiThread {
                        ...
                        tvSt?.text = "Данные синхронизированы"
                    }
                } catch (e: Exception) {}
            }
        })
    ...
    private fun updateUI() {
        ...
        if (cached == null) { fetchYahoo(activeId); return }
        ...
        val set = LineDataSet(entries, "").apply {
            ...
            mode = LineDataSet.Mode.LINEAR // Резкий, "биржевой" график без сглаживания
        }
        
        // Подключаем наши правильные даты к оси X
